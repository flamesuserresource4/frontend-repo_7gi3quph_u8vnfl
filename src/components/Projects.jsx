    ...
    <section id="projects" className="mx-auto max-w-6xl px-6 py-24 text-white">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        {projects.map((p, i) => (
          <motion.div
            key={p.name}
            className="group relative overflow-hidden rounded-xl border border-white/10 bg-slate-900/60 p-5 backdrop-blur-md"
          ...
          >
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {p.stack.map((s) => (
                <span key={s} className="rounded-full border border-white/10 px-2 py-0.5 text-xs text-slate-300">
                  {s}
                </span>
              ))}
            </div>
